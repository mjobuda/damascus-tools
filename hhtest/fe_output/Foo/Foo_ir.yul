object \"Foo\" {
    ...
    object \"runtime\" {
        code {
            function $$external_bar() -> return_val { {
                return_val := Bar_bar(0)
                leave
            } }
            function $$foo() -> return_val { {
                return_val := 42
                leave
            } }
            function Bar_bar(addr) -> return_val {
                let instart := alloc_mstoren(0xfebb0f7e, 4)
            ...
            function Bar_external_foo(addr) -> return_val {
                let instart := alloc_mstoren(0xd9082999, 4)
                let insize := add(4, add(0, 0))
                pop(abi_encode_())
                pop(call(gas(), addr, 0, instart, insize, 0, 0))
                let outsize := returndatasize()
