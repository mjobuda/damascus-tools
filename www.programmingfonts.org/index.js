...
function selectFont() {
  ...
  if (!font) {
    ...
    status_msg.innerHTML =
      'Test drive <a rel="external" href="' +
      font_data[font].website +
      '">' +
      font_data[font].name +
      "!</a>";
  }
  ...
  if (
    typeof font_data !== "undefined" &&
    font_data[font].rendering === "bitmap"
  ) {
  ...
  document
    .querySelectorAll("#select-font [data-alias]")
    .forEach(function (element) {
      element.classList.remove("active");
    });
  document
    .querySelectorAll("#select-font [data-alias='" + font + "']")
    .forEach(function (element) {
...
function applyFilters() {
  ...
  Object.keys(filters).forEach(function (filter) {
    ...
    if (filters[filter]) {
      ...
      button
        .querySelector('svg[alt="' + filters[filter] + '"]')
  ...
  ajax.onreadystatechange = function () {
    ...
    fonts.forEach(function (v) {
      ...
      option.innerHTML =
        ...
        "</span>" +
        "</a>" +
        '<a class="favoritelink" onclick="toggleFavorite(\'' +
        v.alias +
        "')\">" +
        pinIcon +
        "</a>" +
        ...
        v.website +
        '" rel="external"> <span>Website</span>' +
        icon +
        "</a>";
...
function walk(direction) {
  ...
  var next =
    direction === "up"
      ? activeEntry.previousElementSibling
      : activeEntry.nextElementSibling;
  ...
  function isVisible(el) {
    ...
    var container = document
      .querySelector("section.select-list")
      .getBoundingClientRect().height;
    if (direction === "up") {
...
window.addEventListener("DOMContentLoaded", function () {
  var cookieValueSpacing = document.cookie.replace(
    /(?:(?:^|.*;\s*)spacing\s*=\s*([^;]*).*$)|^.*$/,
    "$1"
  );
  var cookieValueSize = document.cookie.replace(
    /(?:(?:^|.*;\s*)size\s*=\s*([^;]*).*$)|^.*$/,
    "$1"
  );
  var cookieValueTheme = document.cookie.replace(
    /(?:(?:^|.*;\s*)theme\s*=\s*([^;]*).*$)|^.*$/,
    "$1"
  );
  var cookieValueLanguage = document.cookie.replace(
    /(?:(?:^|.*;\s*)language\s*=\s*([^;]*).*$)|^.*$/,
    "$1"
  );
