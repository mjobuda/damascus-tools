...
class PythonRepl(PythonInput):
    ...
    def run_and_show_expression(self, expression):
        try:
            # Eval.
            try:
                expression = self.lissp.compile(expression)
                #  print(highlight(">>>",expression.replace("\n", "\n... "), file=sys.stderr))
                print(highlight(">>> "+expression.replace("\n", "\n... "),PythonLexer(), TerminalTrueColorFormatter()))
    ...
    def _format_result_output(self, result: object) -> StyleAndTextTuples:
        ...
        try:
            ...
            formatted_result_repr = to_formatted_text(
                ...
                PygmentsTokens(list(_lex_python_result(result_repr, lexer=SchemeLexer())))
            )
...
def _lex_python_result(tb,lexer = PythonLexer()):
